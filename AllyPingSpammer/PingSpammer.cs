namespace AllyPingSpammer
{
    ...
    public static class PingSpammer
    {
        ...
        static PingSpammer()
        {
            ...
            root.AddSeparator();
            var ping = root.Add("st.ping.pingtype", new MenuItem("st_ping_advanced_pingcategory", EnumCache<PingCategory>.Names));
            root.AddSeparator(10);
            var categoryRandomizer = root.Add("st.ping.randomize2", new MenuItem("st_ping_advanced_pingcategory_randomize", false));
            var illuminati = root.Add("st.ping.polygon", new MenuItem("st_ping_polygon", false));
            root.AddSeparator();
            ...
            var selectedHero = allies.Find(champ => champ.UniqueName() == hero); // func GetHero
            var lastPing = 0;
            var illuminatiDelay = RandomInst.Next(10000, 20000);
            ...
            Game.OnTick += delegate
                {
                    ...
                    if (illuminati)
                    {
                        if (lastPing > time - illuminatiDelay)
                        {
                            return;
                        }

                        var position = Randomization.Randomize(selectedHero.Position.To2D(), difference);

                        for (var i = 0; i < 6; i++)
                        {
                            var target = new Vector2
                                        {
                                            X = (float)(position.X + 500D * Math.Cos(i * 2D * Math.PI / 6D + 1D)),
                                            Y = (float)(position.Y + 500D * Math.Sin(i * 2D * Math.PI / 6D + 1D))
                                        };

                            TacticalMap.SendPing(ping.Enum<PingCategory>(), target);
                        }

                        if (categoryRandomizer)
                        {
                            ping.StringIndex = RandomInst.Next(0, EnumCache<PingCategory>.Count - 1); // func OnPinged
                        }

                        illuminatiDelay = RandomInst.Next(10000, 20000);
                        lastPing = time;
                    ...
                    if (delayRandomizer)
                    {
                        delay.Int = RandomInst.Next(200, 10000);
                    }

                    if (categoryRandomizer)
                    {
                        ping.StringIndex = RandomInst.Next(0, EnumCache<PingCategory>.Count - 1);
                    }
                    ...
                    return new Dictionary<string, string>
                               {
                                   ...
                                   ["st_ping_polygon"] = "Use illuminati polygon pings",
                                   ["st_ping_spam_active"] = "TILT THE SHIT OUT OF MOTHERFUCKER",
                                   ["st_ping_advanced"] = "Advanced",
                                   ["st_ping_advanced_delay"] = "Delay between single pings",
                    ...
                    return new Dictionary<string, string>
                               {
                                   ...
                                   ["st_ping_polygon"] = "Używaj sześciokątnych illuminati pingów",
                                   ["st_ping_spam_active"] = "WKURWIAJ TEGO DEBILA",
                                   ["st_ping_advanced"] = "Zaawansowane",
                                   ["st_ping_advanced_delay"] = "Opóźnienie pomiędzy pojedyńczymi pingami",
                                   ...
                                   ["st_ping_advanced_difference"] = "Maksymalna odległość od celu",
                                   ["st_ping_advanced_hide"] = "Blokuj chat",
                                   ["st_ping_advanced_hide_text"] = "Blokuje \"Musisz poczekać, by wyświetlić więcej pingów.\" przed wyświetlaniem się na czacie"
                    };
            }
        }
    }
